...
export function ordersRoutes(env: Env): Router {
  ...
  // Audit trail for a specific order — requires authorization to view the order
  router.get('/orders/:orderId/audit', requireAuth(env), async (req, res, next) => {
    try {
      ...
      if (!order || order.deletedAt) {
        order = await OrderModel.findOne({ externalOrderId: orderId, deletedAt: null }).lean();
      }
      if (!order) {
        throw new AppError(404, 'ORDER_NOT_FOUND', 'Order not found');
      }

      // Authorization check (same logic as getOrderProof)
      ...
      if (!isPrivileged(roles)) {
        ...
        if (!allowed) {
          throw new AppError(403, 'FORBIDDEN', 'Not allowed to access audit logs');
        }
      }
