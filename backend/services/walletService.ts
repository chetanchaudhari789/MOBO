...
export async function applyWalletCredit(input: WalletMutationInput) {
  ...
  if (externalSession) {
    ...
    // Only log if a new transaction was created (not an idempotent replay)
    if (result) {
      await writeAuditLog({
        action: 'WALLET_CREDIT',
        entityType: 'Wallet',
        entityId: result.walletId ? String(result.walletId) : input.ownerUserId,
        metadata: {
          amountPaise: input.amountPaise,
          type: input.type,
          idempotencyKey: input.idempotencyKey,
          transactionId: String(result._id),
        },
      });
    }
    return result;
