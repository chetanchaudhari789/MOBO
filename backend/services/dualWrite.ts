...
export async function dualWriteOrder(mongoDoc: any): Promise<void> {
  ...
  await safe('Order', async () => {
    ...
    // Only sync if items array is provided and valid
    const hasItems = Array.isArray(mongoDoc.items);
    const mongoItems = hasItems ? mongoDoc.items : [];
    ...
    // If no items in Mongo, ensure no items in PG
    ...
    const campaignIdByMongoId = new Map<string, string>(
